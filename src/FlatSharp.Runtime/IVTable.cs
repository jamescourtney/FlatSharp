...
/// </summary>
[EditorBrowsable(EditorBrowsableState.Never)]
public struct GenericVTable : IVTable
{
    private int offset;
    private int maxIndex;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void Create<TInputBuffer>(TInputBuffer buffer, int offset, out GenericVTable item)
        where TInputBuffer : IInputBuffer
    {
        item = new();

        buffer.InitializeVTable(offset, out item.offset, out item.maxIndex);
        item.offset += 4; // skip past vtable length and table length
    }
