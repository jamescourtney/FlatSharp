...
namespace BenchmarkCore
{
    ...
    public class MeshServerUseCases
    {
        ...
        {
            var maxSize = VoxelRegion3D.Serializer.GetMaxSize(region);
            var networkBuffer = new byte[maxSize];
            VoxelRegion3D.Serializer.Write(networkBuffer, region);
        }
        private void FakeGrpcStreamMeshToClient(Mesh region)
        {
            var maxSize = Mesh.Serializer.GetMaxSize(region);
            var networkBuffer = new byte[maxSize];
            Mesh.Serializer.Write(networkBuffer, region);
        }
        ...
        public void SendRegionToClient()
        {
            ...
            var deserializedRegion = VoxelRegion3D.Serializer.Parse(regionFromStorage);
        ...
        public void SendMeshToClient()
        {
            ...
            var deserializedMesh = Mesh.Serializer.Parse(meshFromStorage);
        ...
        {
            FakeDiskStoredRegion = new byte[VoxelRegion3D.Serializer.GetMaxSize(deserializedRegion)];
            return VoxelRegion3D.Serializer.Write(FakeDiskStoredRegion, deserializedRegion);
        }

        private void SaveToDisk(Mesh mesh)
        {
            FakeDiskStoredMesh= new byte[Mesh.Serializer.GetMaxSize(mesh)];
            Mesh.Serializer.Write(FakeDiskStoredMesh, mesh);
        }
    }
}